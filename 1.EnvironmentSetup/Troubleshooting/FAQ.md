...
* **Scope Verification Logic:** Inspect the `X-OAuth-Scopes` response header. You must scan the comma-separated text string to confirm that `repo`, `workflow`, `write:packages`,`delete:packages` are explicitly present.
...
* **Failure State (Insufficient Privileges):** If the connection succeeds but the `X-OAuth-Scopes` header is missing any of the required scopes (`repo`, `workflow`, `write:packages`,`delete:packages`), stop execution immediately and print:
